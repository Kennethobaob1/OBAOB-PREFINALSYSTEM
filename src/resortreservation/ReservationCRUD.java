...
public class ReservationCRUD {
    ...
    private static void makeReservation() {
        ...
        System.out.print("Enter Reservation Status (Confirmed, Pending, Canceled): ");
    ...
    private static boolean isRoomAvailable(int roomId, String startDate, String endDate) {
        String query = "SELECT COUNT(*) FROM Reservations WHERE room_id = ? AND " +
                       "((start_date <= ? AND end_date >= ?) OR (start_date >= ? AND start_date <= ?))";
        return config.getCount(query, roomId, startDate, endDate, startDate, endDate) == 0;
    }

    private static String getValidStatus(Scanner scanner1) {
        String status;
        do {
            status = scanner.next();
        ...
        } while (!status.equalsIgnoreCase("Confirmed") && 
                 !status.equalsIgnoreCase("Pending") && 
                 !status.equalsIgnoreCase("Canceled"));
        return status;
    }
    ...
    static void viewReservations() {
        config.viewRecords(
            ...
            new String[]{"Reservation ID", "Customer First Name", "Customer Last Name", "Room Type", "Start Date", "End Date", "Status"},
   ...
   private static void updateReservation() {
    ...
    System.out.print("Enter new Status (Confirmed, Pending, Canceled): ");
    ...
    System.out.println("Reservation updated. Please verify the changes.");
}
   ...
   private static void cancelReservation() {
    ...
    System.out.println("Reservation cancellation attempted. Please verify the result.");
}
